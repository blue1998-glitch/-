...
def get_tw_now_str(): 
    return get_tw_now().strftime("%Y-%m-%d %H:%M:%S")
...
def fetch_stock_and_momentum(symbol, market, entry_date_str):
    ...
    try:
        ...
        df_entry = df_all[df_all.index >= entry_date]
        if df_entry.empty: df_entry = df_all.tail(20)
        m_h = round(float(df_entry["High"].max()), 2)
        
        cls = df_all["Close"]
        def ret(d):
            # [修復] 加上長度檢驗與避免除以 0 的防呆機制
            if len(cls) >= d and cls.iloc[-d] != 0:
                return round(((cls.iloc[-1] - cls.iloc[-d]) / cls.iloc[-d]) * 100, 2)
            return 0.0
            
        r_5d = ret(6)
        r_1m = ret(21) if len(cls) >= 21 else r_5d
        r_1q = ret(61) if len(cls) >= 61 else r_1m
...
with tab_portfolio:
        ...
        with st.form("add_stock_form"):
            ...
            if st.form_submit_button("確認建立持倉") and sym:
                new_item = {
                    ...
                    "record_high": price,
                    "realized_pnl": 0,
                    "history": [{"時間": get_tw_now_str(), "動作": "🌱 初始建倉", "成交價": price, "異動股數": f"+{int(shs)}", "剩餘股數": int(shs), "單筆實現損益": "0 元", "備註": f"起始成本 ${price}"}]
                }
    ...
    if not portfolio: st.info("目前尚無持倉，請點擊上方「➕ 新增持股」建立第一檔股票。")
        ...
        for idx, item in enumerate(portfolio):
            ...
            # [修復] 保護舊資料無 history 欄位時發生的 KeyError
            if "history" not in portfolio[idx]:
                portfolio[idx]["history"] = []
                
            history_logs = portfolio[idx]["history"]
            ...
            with st.container():
                ...
                with st.expander(f"⚙️ 操作 {name}（加碼 / 減碼 / 結清）"):
                    ...
                    with op1:
                        st.write("##### 🔼 順勢金字塔加碼")
                        # [修復] 加上 step=0.1，並保護 value 底線，防止 Streamlit 當機
                        add_p = st.number_input("加碼價格", min_value=0.01, step=0.1, value=float(max(cur_price, 0.01)), key=f"ap_{idx}")
                        ...
                        buf = round(((cur_price - sim_avg) / cur_price) * 100, 1) if cur_price > 0 else 0
                        st.caption(f"試算新均價：**${sim_avg}** ｜ 安全緩衝：**{buf:+}%**")
                        
                        if st.button("確認加碼", key=f"ba_{idx}"):
                            portfolio[idx]["history"].append({"時間": get_tw_now_str(), "動作": "🔼 順勢加碼", "成交價": add_p, "異動股數": f"+{int(add_s)}", "剩餘股數": shares + int(add_s), "單筆實現損益": "-", "備註": f"新均價 ${sim_avg} (緩衝 {buf:+}%)"})
                    ...
                    with op2:
                        st.write("##### 🔽 分批減碼")
                        red_p = st.number_input("減碼價格", min_value=0.01, step=0.1, value=float(max(cur_price, 0.01)), key=f"rp_{idx}")
                        ...
                        if st.button("確認減碼", key=f"br_{idx}"):
                            ...
                            portfolio[idx]["history"].append({"時間": get_tw_now_str(), "動作": "🔽 分批減碼", "成交價": red_p, "異動股數": f"-{int(red_s)}", "剩餘股數": new_shares, "單筆實現損益": f"{sim_pnl:+,} 元", "備註": f"報酬率 {sim_roi:+}%"})
                            if new_shares > 0:
