...
def clean_stock_name(name):
    if not name:
        return ""
    raw = str(name).strip()
    if raw in COMMON_NAME_MAP:
        return COMMON_NAME_MAP[raw]
    cleaned = raw
    for suffix in [
        "股份有限公司", "股份公司", "有限股份公司", "有限公司",
        "(股)公司", "（股）公司", "投資控股", "企業", "工業", 
        "科技", "光電", "生技", "製造", "國際", "開發", "控股"
    ]:
        if len(cleaned) > len(suffix) + 1:
...
with tab_portfolio:
    ...
    with st.expander("➕ 新增持股 / 建倉", expanded=False):
        with st.form("add_stock_form"):
            ...
            if submitted and sym:
                ...
                clean_n = clean_stock_name(name.strip()) if name else sym.strip()
                new_item = {
                    "symbol": sym.strip(),
                    "name": clean_n,
                    "market": mkt_code,
    ...
    if not portfolio:
        ...
        for idx, item in enumerate(portfolio):
            sym = item["symbol"]
            name = clean_stock_name(item.get("name", sym))
            mkt = item["market"]
